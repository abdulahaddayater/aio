        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex gap-2 items-center">
            <div className="flex-1 relative">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" strokeWidth={1.75} />
              <input
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search recipes, ingredients, cuisine…"
                className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF6B35] focus:border-transparent text-sm transition"
              />
              {query && (
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 px-3.5 py-2.5 rounded-lg border text-sm font-medium transition flex-shrink-0",
                activeCount > 0
                  ? "bg-[#FF6B35] text-white border-[#FF6B35]"
            ...
            {activeCount > 0 && (
              <button onClick={clearAll} className="text-xs font-medium text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition whitespace-nowrap flex-shrink-0">
                Clear
              </button>
            )}
          </div>
        ...
        <div className="flex gap-7">
          ...
          {showFilters && (
            <div className="lg:hidden fixed inset-0 z-50 flex">
              ...
              <div className="relative ml-auto w-full max-w-[320px] bg-white dark:bg-gray-900 h-full overflow-y-auto shadow-xl">
                <div className="sticky top-0 bg-white dark:bg-gray-900 flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <button onClick={() => setShowFilters(false)}>
                    <X className="w-4.5 h-4.5 text-gray-400" strokeWidth={1.75} />
                  </button>
                </div>
                ...
                <div className="sticky bottom-0 bg-white dark:bg-gray-900 px-5 py-4 border-t border-gray-100 dark:border-gray-800">
                  <button
                    ...
                    className="w-full py-3 bg-[#FF6B35] text-white text-sm font-semibold rounded-xl hover:bg-orange-600 transition"
                  >
